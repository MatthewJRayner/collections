...
export default function FilmMediaCard({
  filmMedia,
  onDelete,
}: FilmMediaCardProps) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 sm:mt-2 text-sm sm:text-base">{filmMedia.format}</p>
      <span
      ...
      {expanded && (
        <div className="mt-t text-left w-full space-y-2 text-sm text-neutral-mid">
          {filmMedia.price && (
            <p>
              <strong>Price:</strong> £{filmMedia.price}
            </p>
          )}
          {filmMedia.date_bought && (
            <p>
              <strong>Purchase Date:</strong> {filmMedia.date_bought}
            </p>
          )}
          {filmMedia.type && (
            <p>
              <strong>Type:</strong> {filmMedia.type}
            </p>
          )}
          {filmMedia.special_features && filmMedia.features && (
            <p>
              <strong>Special Features:</strong> {filmMedia.features.join(", ")}
            </p>
          )}
          {filmMedia.release_year && (
            <p>
              <strong>Release Year:</strong> {filmMedia.release_year}
            </p>
          )}
          {filmMedia.genre?.length ? (
            <p>
              <strong>Genre:</strong> {filmMedia.genre.join(", ")}
            </p>
          ) : null}
          {filmMedia.language && (
            <p>
              <strong>Language:</strong> {filmMedia.language}
            </p>
          )}
          {filmMedia.country && (
            <p>
              <strong>Countries:</strong> {filmMedia.country}
            </p>
          )}
          {filmMedia.studio && (
            <p>
              <strong>Distributor:</strong> {filmMedia.studio}
            </p>
          )}
          {filmMedia.link && (
            <p>
          ...
          {filmMedia.runtime && (
            <p>
              <strong>Runtime:</strong> {filmMedia.runtime}
            </p>
          )}
        </div>
      )}
