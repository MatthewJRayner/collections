...
export default function CastEditor({ crewlist, setCrewlist }: Props) {
  ...
  return (
    <div className="p-2 rounded bg-neutral shadow w-full">
      ...
      >
        <span className={`mr-1 transition ${crewView ? "text-primary" : ""}`}>
          Crew
        </span>
        <span
          className={`transition-all duration-400 ${
            crewView ? "text-primary transform rotate-90" : ""
          }`}
        >
          ▼
        </span>
          ...
          {crewlist.map((member, index) => (
            <div key={index} className="mb-3 bg-background shadow p-2 rounded">
              ...
              <button
                ...
                className="bg-danger text-white px-2 py-1 mt-2 text-sm sm:text-base hover:bg-red-500 transition cursor-pointer active:scale-95"
              >
                Remove
              </button>
            </div>
