  ...
  return (
    ...
    >
      {watch.photo && (
        <div onClick={() => setShowModal(true)} className="cursor-pointer">
          <img
            src={watch.photo}
            alt={watch.model}
            className="h-48 sm:h-64 object-contain mb-2 sm:mb-3"
          />
        </div>
      )}
      ...
      <p className="mt-1 sm:mt-2 text-sm sm:text-base">
        {watch.price ? `£${Number(watch.price).toLocaleString()}` : "—"}
      </p>
      <span
        className={`mt-1 sm:mt-2 px-2 py-1 rounded text-xs ${
          watch.owned ? "bg-green-200 text-success" : "bg-red-200 text-danger"
        }`}
      >
      ...
      {expanded && (
        <div className="mt-4 text-left w-full space-y-2 text-xs sm:text-sm text-neutral-mid">
          ...
          {watch.registration_number && (
            <p>
              <strong>Registration #:</strong> {watch.registration_number}
            </p>
          )}
          {watch.country && (
            <p>
          ...
          {watch.diameter && (
            <p>
              <strong>Diameter:</strong> {watch.diameter} mm
            </p>
          )}
          {watch.movement && (
            <p>
          ...
          {watch.winding && (
            <p>
              <strong>Winding:</strong> {watch.winding}
            </p>
          )}
          {watch.complications?.length ? (
            <p>
              <strong>Complications:</strong> {watch.complications.join(", ")}
            </p>
          ) : null}
          {watch.strap && (
            <p>
              <strong>Strap:</strong> {watch.strap}
            </p>
          )}
          {watch.dial && (
            <p>
              <strong>Dial:</strong> {watch.dial}
            </p>
          )}
          {watch.case && (
            <p>
              <strong>Case:</strong> {watch.case}
            </p>
          )}
          {watch.numerals && (
            <p>
              <strong>Numerals:</strong> {watch.numerals}
            </p>
          )}
          {watch.link && (
            <p>
          ...
          {watch.date_bought && (
            <p>
              <strong>Date Bought:</strong> {watch.date_bought}
            </p>
          )}
        </div>
      )}
