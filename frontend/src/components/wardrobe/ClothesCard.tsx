...
export default function ClothesCard({ clothing, onDelete }: ClothesCardProps) {
  ...
  return (
    <div className={`rounded-lg shadow-md p-4 flex flex-col items-center text-center bg-neutral ${!showModal ? "transition hover:scale-105" : ""}`}>
      ...
      <p className="text-sm text-gray-800">{clothing.colour || "—"}</p>
      <p className="mt-2">{clothing.price ? `£${Number(clothing.price).toLocaleString()}` : "—"}</p>
      <div className="flex space-x-2">
        <span className="mt-2 px-2 py-1 rounded text-xs bg-neutral-mid text-background">{clothing.category ? formatPhrase(clothing.category) : ""}</span>
        <span className="mt-2 px-2 py-1 rounded text-xs bg-neutral-mid text-background">{clothing.type ?? ""}</span>
      </div>
      <span
      ...
      {expanded && (
        <div className="mt-4 text-left w-full space-y-2 text-sm text-neutral-mid">
          {clothing.material && <p><strong>Material:</strong> {clothing.material}</p>}
          {clothing.preferred_quantity && <p><strong>Preferred Quantity:</strong> x{clothing.preferred_quantity}</p>}
          {clothing.features?.length ? (
            <p><strong>Features:</strong> {clothing.features.join(", ")}</p>
          ) : null}
          {clothing.collection && <p><strong>Collection / Year:</strong> x{clothing.collection}</p>}
        </div>
      )}
