...
export default function MusicCard({ music, onDelete }: MusicCardProps) {
    ...
    return (
        <li className={`rounded-lg shadow-md p-4 flex flex-col items-center text-center bg-neutral ${!showModal ? "transition hover:scale-105" : ""}`}>
            ...
            <p className="text-sm text-gray-800">{music.release_date ? `${music.artist} (${music.release_date.substring(0,4)})`: `${music.artist}`}</p>
            <p className="mt-2">{music.price ? `£${Number(music.price).toLocaleString()}` : "-"}</p>
            ...
            {music.tracklist && music.tracklist.length > 0 && (
                <div className="mt-3 w-full flex flex-col">
                    ...
                    >
                        <span className={`mr-1 transition ${showTracks ? "text-primary" : ""}`}>Tracks</span>
                        <span className={`transition duration-400 ${showTracks ? "text-primary transform rotate-180" : ""}`}>▼</span>
                    </button>

                    {showTracks && (
                        <ul className="mt-2 space-y-1 text-sm w-full">
                            {music.tracklist.map((track) => (
                                <li key={track.track_number} className="ml-2 flex justify-between w-full">
            ...
            {expanded && (
                <div className="mt-t text-left w-full space-y-2 text-sm text-neutral-mid">
                    {music.format && <p><strong>Format:</strong> {music.format.charAt(0).toUpperCase() + music.format.slice(1)}</p>}
                    {music.type && <p><strong>Type:</strong> {music.type.charAt(0).toUpperCase() + music.type.slice(1)}</p>}
                    {music.release_date && <p><strong>Release Date:</strong> {music.release_date}</p>}
                    {music.catalog_number && <p><strong>Catalog #:</strong> {music.catalog_number}</p>}
                    {music.genre?.length ? (
                        <p><strong>Genre:</strong> {music.genre.join(", ")}</p>
                    ) : null}
                    {music.length && <p><strong>Length:</strong> {music.length}</p>}
                    {music.language && <p><strong>Language:</strong> {music.language}</p>}
                    {music.country && <p><strong>Country:</strong> {music.country}</p>}
                    {music.label && <p><strong>Label:</strong> {music.label}</p>}
                    {music.link && (
                        <p>
                        <strong>Link:</strong>{" "}
                        <a
