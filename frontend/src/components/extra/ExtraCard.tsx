  ...
  return (
    ...
    >
      ...
      <p className="text-xs sm:text-sm text-gray-800">
        {formatYear({
          year: extra.year,
          year_specificity: Array.isArray(extra.year_specificity)
            ? extra.year_specificity.join(",")
            : extra.year_specificity,
        })}
      </p>
      <p className="mt-1 sm:mt-2 text-sm sm:text-base">
        {extra.price ? `£${Number(extra.price).toLocaleString()}` : "—"}
      </p>
      <span
        className={`mt-1 sm:mt-2 px-2 py-1 rounded text-xs ${
          extra.owned ? "bg-green-200 text-success" : "bg-red-200 text-danger"
        }`}
      >
      ...
      {expanded && (
        <div className="mt-4 text-left w-full space-y-2 text-sm text-neutral-mid">
          ...
          {extra.date_bought && (
            <p>
              <strong>Date Bought:</strong> {extra.date_bought}
            </p>
          )}
        </div>
      )}
