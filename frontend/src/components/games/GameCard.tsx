  ...
  return (
    ...
    >
      ...
      <p className="mt-1 sm:mt-2 text-sm sm:text-base">
        {game.price ? `£${Number(game.price).toLocaleString()}` : "—"}
      </p>
      ...
      <span
        className={`mt-1 sm:mt-2 px-2 py-1 rounded text-xs ${
          game.owned ? "bg-green-200 text-success" : "bg-red-200 text-danger"
        }`}
      >
      ...
      {expanded && (
        <div className="mt-4 text-left w-full space-y-2 text-sm text-neutral-mid">
          {game.platform && (
            <p>
              <strong>Platform:</strong> {game.platform.toUpperCase()}
            </p>
          )}
          {game.publisher && (
            <p>
          ...
          {game.bonus_content?.length ? (
            <p>
              <strong>Bonus Content:</strong> {game.bonus_content.join(", ")}
            </p>
          ) : null}
          {game.language && (
            <p>
          ...
          {game.country && (
            <p>
              <strong>Country:</strong> {game.country}
            </p>
          )}
          {game.genre?.length ? (
            <p>
          ...
          {game.date_bought && (
            <p>
              <strong>Purchase Date:</strong> {formatDate(game.date_bought)}
            </p>
          )}
        </div>
      )}
