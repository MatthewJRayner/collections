...
export default function FilmMediaPage() {
  ...
  const totalOwnedValue = filmMedia
    .filter((f) => f.owned)
    .reduce((sum, f) => sum + Number(f.price || 0), 0);
  const totalValue = filmMedia.reduce(
    (sum, f) => sum + Number(f.price || 0),
    0
  );
  const avgPrice =
    filmMedia.length > 0 ? Number(totalValue / filmMedia.length).toFixed(0) : 0;
  ...
  return (
    <div className="p-4 sm:p-6 flex flex-col w-full min-h-screen">
      <div className="flex justify-center sm:justify-start items-center mb-4 sm:mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold mr-4">Film Collection</h1>
      </div>
